...
created: 2026-05-09
updated: 2026-05-09
tags: [CtnmarketWeb, ctngencargo, 接口替换, 路由迁移, 计划]
...
- `api/CTN_GEN_CARGO/BbCustInfo/Unit`
- `api/CTN_GEN_CARGO/BbCustInfo/Cust`
...
- `api/CTN_GEN_CARGO/CmmCustAcv`
- 以及组织、字典、地区等公共接口

### 集装箱 `container/base_info_edit.vue`

- `api/CTN_GEN_CARGO/CmmCustAcv/Cust`
- `api/client/CMM_CUST_ARCHIVES_QUERY/ALL`
...
1. `api/bb_client/bb_cust_info/unit`
   已替换为：`GET /api/CTN_GEN_CARGO/BbCustInfo/Unit`

2. `API/BB_CLIENT/bb_cust_info/cust`
   已替换为：`GET /api/CTN_GEN_CARGO/BbCustInfo/Cust?custId=...`

3. 提交接口已切换到：`api/CTN_GEN_CARGO/BbCustInfo`

### 集装箱

4. `POST api/client/Cust_Acv`
   已替换为：`POST /api/CTN_GEN_CARGO/CmmCustAcv?id_cust=...&if_acv=...`

5. `GET api/client/Cust_Acv/cust?id_cust=...`
   已替换为：`GET /api/CTN_GEN_CARGO/CmmCustAcv/Cust?id_cust=...`

6. `PUT api/client/Cust_Acv`
   已替换为：`PUT /api/CTN_GEN_CARGO/CmmCustAcv?id_acv=...&if_acv=...`
...
1. 单独核对 `CUSTOMER/PAGE` 与 `CmmCustArchives/Page` 的参数兼容性
2. 单独改造集装箱删除逻辑，确认是按 `id_acv` 还是 `id_cust` 删除
3. 如后端确认可行，再把列表分页切到 `CmmCustArchives/Page`
4. 后续如增加审核按钮，直接接 `CmmCustArchives/Back`

## 本次执行结果

- 已完成件杂货办事处接口替换：`bb_cust_edit.vue` 改为调用 `GET /api/CTN_GEN_CARGO/BbCustInfo/Unit`
- 已完成件杂货详情接口替换：`bb_cust_edit.vue` 改为调用 `GET /api/CTN_GEN_CARGO/BbCustInfo/Cust?custId=...`
- 已完成集装箱新增接口替换：`base_info.vue` 中两个 `POST api/client/Cust_Acv` 调用已改为 `POST /api/CTN_GEN_CARGO/CmmCustAcv`
- 已完成集装箱详情接口替换：`base_info_edit.vue` 改为调用 `GET /api/CTN_GEN_CARGO/CmmCustAcv/Cust?id_cust=...`
- 已完成集装箱修改接口替换：`base_info_edit.vue` 改为调用 `PUT /api/CTN_GEN_CARGO/CmmCustAcv?id_acv=...&if_acv=...`
- 已验证 `yarn build` 通过；当前仅有项目原有的样式深度选择器弃用警告，无新增编译错误

## 当前剩余待办

- `cust_table.vue` 中集装箱删除仍使用 `DELETE api/client/Cust_Acv`，且仍按 `id_cust` 删除，待后端删除语义确认后再改
- `cust_table.vue` 中列表分页仍使用 `GET API/CTN_GEN_CARGO/CUSTOMER/PAGE`，待确认是否可切到 `CmmCustArchives/Page`
- `api/client/CMM_CUST_ARCHIVES_QUERY*`、`api/client/Cust_Config` 及组织/字典类接口暂时保留
